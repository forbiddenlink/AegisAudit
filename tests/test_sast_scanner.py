...
class TestSecretsDetection:
    ...
    def test_google_api_key_detection(self, scanner, temp_project):
        ...
        google_findings = [
            ...
            if "google" in f.title.lower() or "api" in f.description.lower()
        ]
    ...
    def test_private_key_detection(self, scanner, temp_project):
        ...
        key_findings = [
            f
            for f in results.findings
            if "private key" in f.title.lower() or "private key" in f.description.lower()
        ]
...
""")
        ...
        secret_findings = [
            f
            for f in results.findings
            if "secret" in f.id.lower() or "key" in f.description.lower()
        ]
...
""")
        ...
        results = scanner.scan(temp_project)
        static_findings = [
            f for f in results.findings if "eval" in f.id.lower() or "exec" in f.id.lower()
        ]
