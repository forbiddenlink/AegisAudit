...
class TestSecureFlag:
    ...
    def test_secure_flag_present(self, https_artifact, base_config):
        ...
        secure_findings = [
            f
            for f in findings
            if "secure" in f.description.lower() and "missing" in f.description.lower()
        ]
...
class TestHttpOnlyFlag:
    ...
    def test_httponly_present(self, https_artifact, base_config):
        ...
        httponly_findings = [
            f
            for f in findings
            if "httponly" in f.description.lower() and "missing" in f.description.lower()
        ]
...
class TestSameSiteAttribute:
    ...
    def test_samesite_lax(self, https_artifact, base_config):
        ...
        samesite_findings = [
            f
            for f in findings
            if "samesite" in f.description.lower() and "missing" in f.description.lower()
        ]
    ...
    def test_samesite_none_requires_secure(self, https_artifact, base_config):
        ...
        secure_findings = [
            f
            for f in findings
            if ("secure" in f.description.lower() or "samesite" in f.description.lower())
        ]
        assert len(secure_findings) >= 1
