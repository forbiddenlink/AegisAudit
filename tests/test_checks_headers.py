...
class TestHSTSHeader:
    ...
    def test_hsts_short_max_age(self, base_artifact, base_config):
        ...
        findings = check_headers(base_artifact, base_config)
        hsts_findings = [f for f in findings if "HSTS" in f.title or "max-age" in f.description.lower()]
        assert len(hsts_findings) > 0
...
class TestCSPHeader:
    ...
    def test_csp_present(self, base_artifact, base_config):
        ...
        findings = check_headers(base_artifact, base_config)
        csp_findings = [f for f in findings if "missing" in f.description.lower() and "csp" in f.description.lower()]
        assert len(csp_findings) == 0
    
    def test_csp_unsafe_inline(self, base_artifact, base_config):
        base_artifact.headers["content-security-policy"] = "default-src 'self'; script-src 'unsafe-inline'"
        findings = check_headers(base_artifact, base_config)
    ...
    def test_csp_unsafe_eval(self, base_artifact, base_config):
        base_artifact.headers["content-security-policy"] = "default-src 'self'; script-src 'unsafe-eval'"
        findings = check_headers(base_artifact, base_config)
...
class TestXFrameOptions:
    ...
    def test_missing_xfo(self, base_artifact, base_config):
        findings = check_headers(base_artifact, base_config)
        xfo_findings = [f for f in findings if "X-Frame-Options" in f.title or "frame" in f.title.lower()]
    ...
    def test_xfo_deny(self, base_artifact, base_config):
        ...
        findings = check_headers(base_artifact, base_config)
        xfo_findings = [f for f in findings if "X-Frame-Options" in f.title and "missing" in f.description.lower()]
...
class TestReferrerPolicy:
    ...
    def test_missing_referrer_policy(self, base_artifact, base_config):
        findings = check_headers(base_artifact, base_config)
        ref_findings = [f for f in findings if "Referrer-Policy" in f.title or "referrer" in f.title.lower()]
    ...
    def test_referrer_policy_present(self, base_artifact, base_config):
        ...
        findings = check_headers(base_artifact, base_config)
        ref_findings = [f for f in findings if "Referrer-Policy" in f.title and "missing" in f.description.lower()]
...
class TestXContentTypeOptions:
    ...
    def test_missing_xcto(self, base_artifact, base_config):
        findings = check_headers(base_artifact, base_config)
        xcto_findings = [f for f in findings if "X-Content-Type-Options" in f.title or "nosniff" in f.description.lower()]
    ...
    def test_xcto_nosniff(self, base_artifact, base_config):
        ...
        findings = check_headers(base_artifact, base_config)
        xcto_findings = [f for f in findings if "X-Content-Type-Options" in f.title and "missing" in f.description.lower()]
...
class TestPermissionsPolicy:
    ...
    def test_permissions_policy_present(self, base_artifact, base_config):
        ...
        findings = check_headers(base_artifact, base_config)
        pp_findings = [f for f in findings if "Permissions-Policy" in f.title and "missing" in f.description.lower()]
...
class TestCrossOriginPolicies:
    ...
    def test_missing_coop(self, base_artifact, base_config):
        findings = check_headers(base_artifact, base_config)
        coop_findings = [f for f in findings if "Cross-Origin-Opener-Policy" in f.title or "COOP" in f.title]
    ...
    def test_missing_coep(self, base_artifact, base_config):
        findings = check_headers(base_artifact, base_config)
        coep_findings = [f for f in findings if "Cross-Origin-Embedder-Policy" in f.title or "COEP" in f.title]
        assert isinstance(coep_findings, list)
    
    def test_missing_corp(self, base_artifact, base_config):
        findings = check_headers(base_artifact, base_config)
        corp_findings = [f for f in findings if "Cross-Origin-Resource-Policy" in f.title or "CORP" in f.title]
...
class TestInfoLeakage:
    ...
    def test_server_header_present(self, base_artifact, base_config):
        ...
        findings = check_headers(base_artifact, base_config)
        server_findings = [f for f in findings if "server" in f.title.lower() or "server" in f.description.lower()]
    ...
    def test_x_powered_by_present(self, base_artifact, base_config):
        ...
        findings = check_headers(base_artifact, base_config)
        xpb_findings = [f for f in findings if "powered" in f.description.lower() or "X-Powered-By" in f.title]
